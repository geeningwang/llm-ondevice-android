...
| **平均内存占用 (PSS)** | 623.4 MB | 1123.6 MB | 4248.9 MB |
| **峰值内存占用 (PSS)** | **1083.79 MB** | **1185.34 MB** | **4342.78 MB** |
| **平均 Native 堆大小** | 238.4 MB | 792.1 MB | 2061.2 MB |
| **峰值 Native 堆大小** | **508.18 MB** | **795.64 MB** | **2095.61 MB** |
| **峰值 CPU 负载 (多核扩展)** | 283.78% (3 核并发) | 305.89% (3 核并发) | 302.06% (3 核并发) |
| **峰值流式解码速度** | **13.68 tok/s** | **7.28 tok/s** | **5.48 tok/s** |
...
1. **内存分级划分 (PSS 进程实际内存 vs. Native 堆分配)**:
   ...
   - **Gemma 4 E2B-IT**：具备极佳的模型内存映射参数。虽磁盘体积达 ~2.41 GB，但 LiteRT-LM 的 dynamic `mmap` 动态虚存映射只激活了 **795.64 MB** 的 Native 堆物理缓存，总 PSS RAM 仅为 **1.18 GB**。普通主流手机（6GB-8GB 运存）完全能流畅承载。
   - **Gemma 4 E4B-IT**：高开销、高性能选项。其峰值 Native 堆直接霸占 **2.09 GB** 物理运存，驱动总进程 PSS 达到了 **4.34 GB** 的庞大开销。若在生产环境部署 E4B，设备要求底线为 **8GB - 12GB+ 运存的旗舰机型**，否则会被 Android 的 Low Memory Killer Daemon (`lmkd`) 后台静默强制杀死。

2. **多核并行与解码效率**:
   - 在流式解码或 Prompt prefill 的高算力要求阶段，三款引擎均自动调用了物理设备上的 **3 颗 CPU 核心并发运行**（CPU 峰值均达到 **280% - 306%** 极高算力扩张）。
   - 实测 CPU 吞吐表明，**Gemma 3 1B-IT** 可提供流畅无比的 **13.68 tok/s** 爆发打字机效果，**Gemma 4 E2B-IT** 为 **7.28 tok/s**，而 **Gemma 4 E4B-IT** 在 CPU 上依然可以榨出 **5.48 tok/s** 稳定文字产出。
